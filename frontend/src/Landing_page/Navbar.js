    ...
    const fetchCounts = async () => {
      try {
        const [gigRes, serviceRes] = await Promise.all([
          axios.get(
            `https://taskora-backend-aejx.onrender.com/count/gigs/${encodeURIComponent(normalizedCity)}`,
          ),
          axios.get(
            `https://taskora-backend-aejx.onrender.com/count/services/${encodeURIComponent(normalizedCity)}`,
          ),
        ]);
