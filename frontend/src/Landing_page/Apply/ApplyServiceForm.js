...
// import "./ApplyGigForm.css"; // ✅ reuse same CSS
...
//     try {
//       await axios.post(`http://localhost:3002/applyService/${serviceId}`, data, {
...
//     try {
//       await axios.post(
//         `http://localhost:3002/applyService/${serviceId}`,
//         data,
//         {
//           withCredentials: true,
//           headers: { "Content-Type": "multipart/form-data" },
...
const ApplyServiceForm = () => {
  ...
  const handleChange = (e) => {
    ...
    // ✅ ONLY ADDITION (contact restriction)
    else if (name === "contact") {
      const cleaned = value.replace(/\D/g, "");
      setFormData((prev) => ({
        ...prev,
        contact: cleaned.slice(0, 10),
      }));
    } 
    else {
      setFormData({ ...formData, [name]: value });
    ...
    <div className="apply-container">
      <div className="apply-card">
        ...
        <form onSubmit={handleSubmit} className="apply-form">
          ...
          {/* ✅ LABEL ADDED */}
          <label>Proof of Previous Work </label>
          <input
            type="file"
            name="pictures"
            accept="image/*"
